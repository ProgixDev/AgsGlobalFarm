 ...
 * plugin in this folder for the same constraint. A hand-edited
 ...
 *
 * Reads four env vars, loaded by Expo CLI itself from `mobile/.env` (no
 * `EXPO_PUBLIC_` prefix — that prefix inlines a value into the CLIENT BUNDLE,
 * which a signing password must never do):
 *   AGS_RELEASE_STORE_FILE      — path to the .jks, relative to android/app/
 *                                  (e.g. ../../keystores/ags-release.jks)
 *   AGS_RELEASE_STORE_PASSWORD
 *   AGS_RELEASE_KEY_ALIAS
 *   AGS_RELEASE_KEY_PASSWORD
 *
 * Absent (e.g. CI without the secret, or a machine that never set up local
 * signing), release silently falls back to `signingConfigs.debug` — the same
 * behavior the stock template ships with. This plugin only ever ADDS a path,
 * never removes the one that was already there.
 */
module.exports = function withReleaseSigning(config) {
  config = withGradleProperties(config, (config) => {
    const values = {
      AGS_RELEASE_STORE_FILE: process.env.AGS_RELEASE_STORE_FILE,
      AGS_RELEASE_STORE_PASSWORD: process.env.AGS_RELEASE_STORE_PASSWORD,
      AGS_RELEASE_KEY_ALIAS: process.env.AGS_RELEASE_KEY_ALIAS,
      AGS_RELEASE_KEY_PASSWORD: process.env.AGS_RELEASE_KEY_PASSWORD,
    };
  ...
  config = withAppBuildGradle(config, (config) => {
    ...
    if (
      !contents.includes(signingConfigsMarker) ||
      !contents.includes(releaseBuildTypeMarker)
    ) {
      ...
      throw new Error(
        "withReleaseSigning: expected build.gradle shape not found — the Expo prebuild template changed. Update the markers in mobile/plugins/withReleaseSigning.js.",
      );
    }
    ...
    const releaseSigningBlock =
      "    signingConfigs {\n" +
      "        release {\n" +
      "            if (project.hasProperty('AGS_RELEASE_STORE_FILE')) {\n" +
      "                storeFile file(AGS_RELEASE_STORE_FILE)\n" +
      "                storePassword AGS_RELEASE_STORE_PASSWORD\n" +
      "                keyAlias AGS_RELEASE_KEY_ALIAS\n" +
      "                keyPassword AGS_RELEASE_KEY_PASSWORD\n" +
      "            }\n" +
      "        }\n" +
      "        debug {";
    ...
    const releaseBuildTypeReplacement =
      "        release {\n" +
      "            signingConfig project.hasProperty('AGS_RELEASE_STORE_FILE') ? signingConfigs.release : signingConfigs.debug";
